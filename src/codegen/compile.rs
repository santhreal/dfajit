...
pub fn compile_x86_64(table: &TransitionTable, output_links: &[u32]) -> Result<ExecutableBuffer> {
    ...
    c.extend_from_slice(&[0x49, 0x89, 0xFC, 0x48, 0x89, 0xF5, 0x49, 0x89, 0xD6, 0x48, 0x89, 0xCB]);
    ...
    c.extend_from_slice(&[0; 8]);
    c.extend_from_slice(&[
        0x42, 0x8B, 0x04, 0x87, 0x4C, 0x89, 0xFF, 0x48, 0x6B, 0xFF, 0x10, 0x4C, 0x01, 0xF7,
        0x89, 0x07, 0x89, 0xC1,
    ]);
    ...
    c.extend_from_slice(&[0; 8]);
    c.extend_from_slice(&[
        0x8B, 0x0C, 0x8A, 0x44, 0x89, 0xEA, 0x83, 0xC2, 0x01, 0x89, 0x57, 0x08, 0x29, 0xCA,
        0x73, 0x02, 0x31, 0xD2, 0x89, 0x57, 0x04,
    ]);
    ...
    c.extend_from_slice(&[0x49, 0xFF, 0xC5, 0x43, 0x0F, 0x18, 0x44, 0x2C, 0x40, 0x49, 0x39, 0xED, 0x0F, 0x82]);
    ...
    c.extend_from_slice(&[0x4C, 0x89, 0xF8, 0x41, 0x5F, 0x41, 0x5E, 0x41, 0x5D, 0x41, 0x5C, 0x5D, 0x5B, 0xC3]);
...
fn compile_interpreted_fallback(
    ...
    unsafe {
        std::ptr::copy_nonoverlapping(FALLBACK_CODE.as_ptr(), ptr.cast::<u8>(), FALLBACK_CODE.len());
    }
